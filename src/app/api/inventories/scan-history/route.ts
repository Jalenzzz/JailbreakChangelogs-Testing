...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!INVENTORY_API_URL) {
      return NextResponse.json({ error: 'Inventory API URL not configured' }, { status: 500 });
    }

    const response = await fetch(`${INVENTORY_API_URL}/user/history?id=${userId}&limit=1000`, {
      method: 'GET',
      headers: {
