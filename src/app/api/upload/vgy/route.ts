 ...
 */
function validateExtensionMimeMatch(fileName: string, mimeType: string): boolean {
    ...
    if (buffer.length >= pattern.length) {
      ...
      if (matches) {
        console.warn('Suspicious file content detected:', {
          pattern: pattern.map((b) => '0x' + b.toString(16).toUpperCase()).join(' '),
          mimeType,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!validateExtensionMimeMatch(file.name, file.type)) {
      ...
      return NextResponse.json({ message: 'File type mismatch detected' }, { status: 400 });
    }
    ...
    if (!isValidImage) {
      ...
      return NextResponse.json({ message: 'Invalid file content' }, { status: 400 });
    }
    ...
    if (!validateFileContent(uint8Array, file.type)) {
      ...
      return NextResponse.json({ message: 'File content validation failed' }, { status: 400 });
    }
    ...
    if (result.error) {
      return NextResponse.json({ message: 'Upload failed: ' + result.error }, { status: 400 });
    }
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      return NextResponse.json({ message: 'Upload timeout - please try again' }, { status: 408 });
    }
