...
export async function POST(request: Request) {
  ...
  if (!token || !following) {
    return NextResponse.json({ message: 'Unauthorized or missing following' }, { status: 400 });
  }
  ...
  if (!upstream.ok) {
    ...
    return NextResponse.json({ message: 'Failed to unfollow user' }, { status: upstream.status });
  }
