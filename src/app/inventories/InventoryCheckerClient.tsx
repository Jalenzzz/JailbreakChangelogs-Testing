...
export default function InventoryCheckerClient({
  ...
  useEffect(() => {
    ...
    if (scanWebSocket.status === 'error') {
      ...
      if (scanWebSocket.error && scanWebSocket.error.includes('No bots available')) {
        showScanErrorToast('No scan bots are currently online. Please try again later.');
      } else if (
        scanWebSocket.message &&
        ...
        const tierNames = ['Free', 'Supporter I', 'Supporter II', 'Supporter III'];
    ...
    if (scanWebSocket.forceShowError && scanWebSocket.error && !forceShowErrorHandledRef.current) {
  ...
  useEffect(() => {
    if (refreshedData) {
      toast.success(`Data refreshed successfully! Updated ${refreshedData.item_count} items`, {
        duration: 4000,
          ...
          <div className="border-border-primary bg-secondary-bg shadow-card-shadow rounded-lg border p-6">
            <div className="text-center">
              ...
              <h3 className="text-status-error mb-2 text-lg font-semibold">
                {error.includes('Server error') ? 'Server Error' : 'Unable to Fetch Inventory Data'}
              </h3>
              ...
              {isOwnInventory ? (
                    ...
                    <div className="space-y-3">
                      <div className="text-center">
                        <p className="text-secondary-text text-sm">
                          Wait for one of our bots to randomly join your trade server
                        </p>
                      </div>
                      <div className="text-secondary-text text-center text-sm font-medium">OR</div>
                      <div className="flex justify-center">
                        <button
                          onClick={() => {
                            console.log('[INVENTORY] Request Scan button clicked');
                            console.log('[INVENTORY] Current scan status:', scanWebSocket.status);
                            console.log('[INVENTORY] Current scan error:', scanWebSocket.error);
                            ...
                            <>
                              ...
                              scanWebSocket.message.includes('recent scan') ? (
                                <>
                                  ...
                                  {scanWebSocket.message.includes('recent scan')
                                    ? 'Recent Scan Found'
                                    : scanWebSocket.message.includes('User not found')
                                      ? 'User Not Found'
                                      : scanWebSocket.message.includes('not in game')
                                        ? 'Not In Game'
                                        : scanWebSocket.message}
                                </>
                              ) : scanWebSocket.message &&
                ...
                <div className="border-border-primary bg-secondary-bg shadow-card-shadow mt-4 rounded-lg border p-4">
                  ...
                  <p className="text-secondary-text text-sm">Login to request an inventory scan.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <>
            ...
            <div className="mt-6">
              <InventoryOverflowTabs
                value={activeTab}
                onChange={(e, idx) => handleTabChange(e as unknown as React.SyntheticEvent, idx)}
