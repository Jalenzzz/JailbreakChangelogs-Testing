...
import { FollowingData, FollowerData, UserSettings, UserFlag } from '@/types/auth';
...
async function AdditionalDataFetcher({ userId, user }: { userId: string; user: User }) {
  try {
    ...
    const [
      ...
      fetch(`${BASE_API_URL}/users/description/get?user=${userId}&nocache=true`),
    ...
    // Process responses
    const followersData: FollowerData[] | string = await followersResponse.json();
    const followingData: FollowingData[] | string = await followingResponse.json();
