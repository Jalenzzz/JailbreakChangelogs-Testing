...
export async function fetchFavoriteItemDetails(favorites: Array<{ item_id: string }>) {
  try {
    ...
    const itemDetailsPromises = favorites.map(async (favorite) => {
      try {
        ...
        if (isSubItem) {
          ...
          const response = await fetch(`${BASE_API_URL}/items/get/sub?parent_id=${parentId}`);
          if (response.ok) {
        ...
        console.error('[SERVER ACTION] Error fetching item details for %s:', favorite.item_id, err);
        return { [favorite.item_id]: null };
      }
    });
    ...
    console.error('[SERVER ACTION] Failed to fetch favorite item details:', error);
    return {};
  }
}
...
export async function fetchCommentDetails(
  ...
  try {
    ...
    const [changelogs, items, seasons] = await Promise.all([
      Promise.all(changelogIds.map((id) => fetchChangelog(id).catch(() => null))),
