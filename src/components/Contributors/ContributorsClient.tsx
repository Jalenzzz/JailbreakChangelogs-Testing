...
export default function ContributorsClient({ usersWithFlags }: ContributorsClientProps) {
  ...
  const sortByHierarchy = (users: (UserWithFlags & { role: string })[]) => {
    return users.sort((a, b) => {
      const aPrimaryFlag = a.flags
        ...
        .reduce((highest, current) => (current.index < highest.index ? current : highest));
    ...
    <div className="bg-primary-bg">
      <div className="container mx-auto px-6 py-10">
        ...
        <p className="text-secondary-text mx-auto my-6 max-w-2xl text-center">
          Our dedicated team of developers, designers, and contributors who make Jailbreak
          Changelogs possible.
        </p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 md:grid-cols-2 xl:mt-16 xl:grid-cols-3">
          ...
          {staticContributorsToShow.map((contrib) => renderStaticContributor(contrib))}
        </div>
      </div>
    </div>
  );
