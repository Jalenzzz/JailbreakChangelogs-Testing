...
export default function CrewSeasonSelector({ currentSeason }: CrewSeasonSelectorProps) {
  ...
  return (
    <div className="mb-6">
        ...
        {selectLoaded ? (
          <Select
            ...
            classNames={{
              ...
              dropdownIndicator: () => 'text-secondary-text hover:text-primary-text cursor-pointer',
            }}
          />
        ) : (
      ...
      {/* Season Info */}
      <div className="text-secondary-text mt-3 text-sm">{getSeasonDescription(currentSeason)}</div>
    </div>
  );
}
