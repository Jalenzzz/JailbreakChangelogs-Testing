  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    return [...mappedItems].sort((a, b) => {
      switch (sortOrder) {
        ...
        case 'created-asc':
          const aCreatedAsc = a.item.info.find((info) => info.title === 'Created At')?.value;
          const bCreatedAsc = b.item.info.find((info) => info.title === 'Created At')?.value;
          if (aCreatedAsc && bCreatedAsc) {
            return new Date(aCreatedAsc).getTime() - new Date(bCreatedAsc).getTime();
          }
          return 0;
        case 'created-desc':
          const aCreatedDesc = a.item.info.find((info) => info.title === 'Created At')?.value;
          const bCreatedDesc = b.item.info.find((info) => info.title === 'Created At')?.value;
          ...
          <div className="text-secondary-text space-y-1 text-sm">
            ...
            <div className="mt-2 text-xs">
              ...
              {inventoryStats.duplicates
                ...
                .reduce(
                  (acc, curr, index) => acc.concat(index === 0 ? [curr] : [', ', curr]),
                  [] as React.ReactNode[],
                )}
            </div>
          </div>
      ...
      {!isFiltering && filteredAndSortedItems.length > 0 && (
        <div className="border-button-info bg-button-info/10 mb-4 rounded-lg border p-3">
          <div className="text-primary-text flex items-start gap-2 text-sm">
            <Icon icon="emojione:light-bulb" className="text-button-info flex-shrink-0 text-lg" />
