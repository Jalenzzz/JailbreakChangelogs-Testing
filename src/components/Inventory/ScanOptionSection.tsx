...
export default function ScanOptionSection({
  ...
  return (
    ...
    >
      <div className={`flex items-start gap-3 ${isOwnInventory ? 'justify-center' : ''}`}>
        {!isOwnInventory && (
        ...
        <div className={`${isOwnInventory ? 'flex justify-center' : 'min-w-0 flex-1'}`}>
          {!isOwnInventory && (
            <>
          ...
          <div className={`flex ${isOwnInventory ? 'justify-center' : 'gap-2'}`}>
            {!isAuthenticated ? (
              <Link
                href="/faq"
