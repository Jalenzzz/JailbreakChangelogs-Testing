...
export default function UserRefreshSection({
  ...
  const isOwnInventory = isAuthenticated && user?.roblox_id === currentData?.user_id;
    ...
    <>
      ...
      {isOwnInventory ? (
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-white">
                  {isDataFresh() ? (
                    <span className="text-green-400" aria-label="Data is fresh and up to date">
                      Fresh data
                    </span>
                  ) : (
                    <span className="text-yellow-400" aria-label="Data may be outdated">
        ...
        <div className="mt-4 rounded-lg bg-[#2E3944] p-4">
          <p className="text-muted mb-1 text-sm font-medium">Are you the owner of this profile?</p>
          <p className="text-sm text-[#FFFFFF]">
            Login to request an inventory scan. Your inventory will be automatically scanned when
            you join a trading server.
          </p>
        </div>
      )}
      ...
      {refreshedData && (
        <div className="rounded-lg border border-green-500/20 bg-green-900/20 p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-green-400">Data refreshed successfully!</span>
          </div>
          <p className="mt-1 text-xs text-green-300">Updated {refreshedData.item_count} items</p>
        </div>
      )}
