...
export default function ItemCard({ item, isFavorited, onFavoriteChange }: ItemCardProps) {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    if (
      ...
      (e.target.closest('button') || e.target.closest('a') || e.target.closest('.dropdown'))
    ) {
      return;
    }
      ...
      if (variant) {
        ...
        if (matchingSubItem) {
          ...
          const defaultVariant =
            item.children.find((child) => child.sub_name === '2023') || item.children[0];
      ...
      >
        {hasChildren && item.children && (
          <div className="absolute top-2 right-2 z-20">
            <SubItemsDropdown onSelect={setSelectedSubItem} selectedSubItem={selectedSubItem}>
              {item.children}
            </SubItemsDropdown>
          </div>
        )}
