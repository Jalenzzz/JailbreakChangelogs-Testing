...
const ItemValueChart = ({ itemId, variantId, hideTradingMetrics = false }: ItemValueChartProps) => {
  ...
  const dupedSeries = filteredData.map((item) => parseRawValue(item.duped_value));
  ...
  const shouldShowValueChart = hasNumericFor(cashSeries) || hasNumericFor(dupedSeries);
      ...
      <div>
        ...
        {shouldShowValueChart ? (
          ...
          <div className="bg-secondary-bg rounded-lg p-8 text-center">
            ...
            <p className="text-secondary-text mx-auto max-w-md text-sm leading-relaxed">
              This item doesn&apos;t have any recorded value changes yet. Value history will appear
              here once the item&apos;s value is updated.
            </p>
          </div>
        )}
      </div>
