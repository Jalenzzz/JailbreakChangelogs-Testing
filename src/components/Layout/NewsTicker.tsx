...
export default function NewsTicker() {
  ...
  const handleDismiss = () => {
    ...
    safeLocalStorage.setItem('inventory-release-announcement-dismissed', 'true');
  };

  useEffect(() => {
    // Check if user has previously dismissed this announcement
    const dismissed = safeLocalStorage.getItem('inventory-release-announcement-dismissed');
    if (dismissed === 'true') {
      setIsVisible(false);
    }
  }, []);
        ...
        <div className="relative flex flex-col items-center justify-center gap-3 pr-8 lg:flex-row lg:gap-4 lg:pr-12">
          ...
          <div className="flex flex-col items-center gap-2 lg:flex-row lg:gap-3">
            <span className="text-primary-text text-center text-xs lg:text-sm">
              <strong>Inventory Checker!</strong> Check your own inventory or browse other
              players&apos; inventories to see their items and values
            </span>
          </div>
