...
export default function UserRankDisplay() {
  ...
  useEffect(() => {
    const fetchUserRank = async () => {
      ...
      setIsLoading(true);
      try {
        const response = await fetch(`/api/money/rank?user_id=${user.roblox_id}`);
        if (response.ok) {
    ...
    return (
      <div className="border-border-primary hover:border-border-focus bg-secondary-bg mb-6 rounded-lg border p-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-primary-text text-lg font-semibold">Your Rank</h3>
  ...
  return (
    <div className="mb-6">
      <div className="mb-4">
        <h2 className="text-primary-text text-lg font-bold sm:text-xl">Your Rank</h2>
