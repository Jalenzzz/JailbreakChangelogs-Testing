...
export default function OGStats({ initialData }: OGStatsProps) {
  ...
  const ogStats = (() => {
    ...
    const originalOwnerItems = initialData.results.filter((item) => item.isOriginalOwner).length;
    const totalTrades = initialData.results.reduce((sum, item) => sum + item.timesTraded, 0);
    ...
    <div className="rounded-lg border border-[#2E3944] bg-[#212A31] p-6 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-lg bg-[#37424D] p-4">
          ...
          <p className="text-2xl font-bold text-white">{ogStats.originalOwnerItems}</p>
        </div>
      ...
      {ogStats.originalOwnerItems > 0 && (
        <div className="mt-4 rounded-lg border border-green-500/20 bg-green-900/20 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-medium text-green-400">OG Player Detected!</h3>
              <p className="mt-1 text-sm text-green-300">
                This user has {ogStats.originalOwnerItems} original items, making them an OG player.
                These are items they originally owned before trading.
              </p>
            </div>
          </div>
        </div>
