...
import { CircularProgress, Button, IconButton, Menu, MenuItem, Tooltip } from '@mui/material';
import { useVirtualizer } from '@tanstack/react-virtual';
import { CommentData } from '@/utils/api';
import { refreshComments, fetchUsersBatchAction } from '@/app/api/comments/actions';
...
const ChangelogComments: React.FC<ChangelogCommentsProps> = ({
  ...
  const refreshCommentsFromServer = useCallback(async () => {
    ...
    try {
      ...
      const commentsArray = Array.isArray(result.data?.comments) ? result.data.comments : [];
        ...
        <div className="flex flex-col gap-4">
          ...
          <form onSubmit={handleSubmitComment} className="mb-2">
            <div className="space-y-2">
              ...
              />
              {!isLoggedIn && (
                <p className="text-secondary-text text-xs">You must be logged in to comment</p>
              )}
            </div>
                                  ...
                                  <>
                                    <div className="prose prose-sm max-w-none">
                                      {(() => {
                                        ...
                                        const isLongLine =
                                          comment.content.length > MAX_VISIBLE_CHARS;
