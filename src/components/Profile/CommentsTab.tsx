    ...
    ? activeFilter === 'item'
      ? // For "item" filter, show all types that aren't in the main categories
        sortedComments.filter(
          (comment) => !MAIN_CATEGORIES.includes(comment.item_type.toLowerCase()),
        )
      : // For specific filters, show only that type (case-insensitive)
        sortedComments.filter(
          (comment) => comment.item_type.toLowerCase() === activeFilter.toLowerCase(),
        )
    : sortedComments;
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {currentComments.map((comment) => (
                    <Comment
                      key={comment.id}
                      {...comment}
                      changelogDetails={commentDetails.changelogs[comment.item_id.toString()]}
                      itemDetails={commentDetails.items[comment.item_id.toString()]}
                      seasonDetails={commentDetails.seasons[comment.item_id.toString()]}
                      tradeDetails={commentDetails.trades[comment.item_id.toString()]}
