...
export default function FavoritesTab({
  ...
  const [loading, setLoading] = useState(!serverFavorites.length && isLoadingAdditionalData);
      ...
      try {
        ...
        if (Array.isArray(serverFavorites) && serverFavorites.length > 0) {
          ...
          const favoritesWithDetails = serverFavorites.map((favorite) => {
            const itemDetails =
              sharedItemDetails[favorite.item_id] || favoriteItemDetails[favorite.item_id];
            return {
              ...favorite,
              details: itemDetails ? { item: itemDetails as ItemDetails } : undefined,
            };
          });
        ...
        setError(err instanceof Error ? err.message : 'Failed to process favorites');
      }
    };
  ...
  }, [userId, shouldHideFavorites, serverFavorites, favoriteItemDetails, sharedItemDetails]);
        ...
        <div className="border-border-primary rounded-lg border p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-primary-text text-lg font-semibold">Favorited Items</h2>
            </div>
          </div>
