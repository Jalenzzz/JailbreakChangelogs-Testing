...
import { CircularProgress, Box, Pagination, Chip, Skeleton } from '@mui/material';
...
export default function TradeAdsTab({
  ...
  const renderTradeItem = (
    ...
    return (
      <div
        key={itemData.id}
        className="rounded-lg border p-3 shadow-sm transition-colors hover:border-[#5865F2]"
      >
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          {ad.offering.length > 0 ? (
            <div className="space-y-2">{renderTradeItem(ad.offering[0], ad.offering.length)}</div>
          ) : (
