...
import { CircularProgress, Box, Pagination, Chip, Skeleton, Tooltip } from '@mui/material';
...
export default function TradeAdsTab({
  ...
  const TradeAdItem = ({ ad }: { ad: TradeAd }) => {
    ...
    const expiresTime = useOptimizedRealTimeRelativeDate(ad.expires, `trade-ad-expires-${ad.id}`);
  ...
  const renderTradeAd = (ad: TradeAd & { createdTime: string; expiresTime: string }) => {
    return (
      <div
        key={ad.id}
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="border-status-success/20 bg-status-success/10 rounded-lg border p-4">
            <h3 className="text-status-success mb-4 text-lg font-bold">Offering</h3>
          ...
          <div className="border-status-error/20 bg-status-error/10 rounded-lg border p-4">
            <h3 className="text-status-error mb-4 text-lg font-bold">Requesting</h3>
      ...
      <div className="space-y-6">
        <div className="border-border-primary rounded-lg border p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="text-primary-text text-lg font-semibold">Trade Ads</h2>
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="border-border-primary rounded-lg border p-4">
        <div className="mb-3 flex items-center gap-2">
          <SwapHorizIcon className="text-button-info" />
          <h2 className="text-primary-text text-lg font-semibold">Trade Ads [{tradeAds.length}]</h2>
        </div>
        ...
        {tradeAds.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <h3 className="text-primary-text mb-2 text-xl font-semibold">
              {isOwnProfile ? 'You have not posted any trade ads.' : 'No trade ads available.'}
            </h3>
            {isOwnProfile && (
              <Link
                href="/trading"
