...
export default function SeasonDetailsClient({
  ...
  const filteredSeasonList = seasonList
    ...
    .sort(
      (a: { season: number; title: string }, b: { season: number; title: string }) =>
        b.season - a.season,
    );
  ...
  const handleSeasonSelect = async (selectedId: string) => {
    ...
    const selectedSeason = seasonList.find(
      (season) => season.season.toString() === selectedId || season.season === parseInt(selectedId),
    );
          ...
          <div className="sm:col-span-12 xl:col-span-8">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-secondary-text text-sm">
                      Calculate how long it will take to reach your target level and see if you can
            ...
            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="border-stroke bg-secondary-bg rounded-lg border p-4">
                <h3 className="text-primary-text mb-2 font-semibold">Start Date</h3>
                <p className="text-secondary-text">
                  {startDate ? formatProfileDate(startDate.getTime()) : 'TBD'}
                </p>
              </div>
              <div className="border-stroke bg-secondary-bg rounded-lg border p-4">
                <h3 className="text-primary-text mb-2 font-semibold">End Date</h3>
                <p className="text-secondary-text">
                  {endDate ? formatProfileDate(endDate.getTime()) : 'TBD'}
                </p>
              </div>
              <div className="border-stroke bg-secondary-bg rounded-lg border p-4">
                <h3 className="text-primary-text mb-2 font-semibold">Duration</h3>
                <p className="text-secondary-text">
                  {startDate && endDate
                ...
                .map((reward) => (
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between">
                      <h3 className="text-primary-text font-semibold">{reward.item}</h3>
                    ...
                    <p className="text-secondary-text text-sm">Requirement: {reward.requirement}</p>
                  </div>
                ))}
            </div>
          </div>
