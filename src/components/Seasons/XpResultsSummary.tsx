...
export default function XpResultsSummary({ results, season }: XpResultsSummaryProps) {
  ...
  const doubleXpStartTime = new Date(results.importantDates.doubleXpStart).getTime();
  const seasonEndTime = new Date(results.importantDates.seasonEnds).getTime();
  const isDoubleXpLive = currentTime >= doubleXpStartTime && currentTime <= seasonEndTime;

  const [xpMode, setXpMode] = useState<'normal' | 'double'>(isDoubleXpLive ? 'double' : 'normal');
      ...
      return {
        noPass: results.doubleXpResults?.noPass
          ? {
              ...
              days: Math.ceil(
                (new Date(results.doubleXpResults.noPass.completionDate).getTime() - Date.now()) /
                  (1000 * 60 * 60 * 24),
              ),
            }
          : undefined,
        withPass: results.doubleXpResults?.withPass
          ? {
              ...
              days: Math.ceil(
                (new Date(results.doubleXpResults.withPass.completionDate).getTime() - Date.now()) /
                  (1000 * 60 * 60 * 24),
              ),
            }
          : undefined,
  ...
  const getOverallStatus = () => {
    ...
    if (canReachWithoutPass && canReachWithPass) {
      return {
        ...
        icon: <Icon icon="mdi:party-popper" className="text-green-400" inline={true} />,
      };
      ...
      return {
        ...
        icon: <Icon icon="si:alert-fill" className="text-yellow-400" inline={true} />,
      };
    } else if (!canReachWithoutPass && !canReachWithPass) {
      return {
        type: 'error',
    ...
    return {
      ...
      icon: <Icon icon="mdi:chart-line" className="text-blue-400" inline={true} />,
    };
  };
      ...
      <div className="border-border-primary hover:border-border-focus bg-secondary-bg rounded-lg border p-6">
        <div className="mb-6 text-center">
          <div className="mb-2 flex items-center justify-center gap-2">
            ...
            <h3 id="season-progress-summary" className="text-primary-text text-2xl font-bold">
              {status.message}
            </h3>
          </div>
        ...
        <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="text-center">
            ...
            <div className="text-primary-text text-xl font-bold">{results.currentLevel}</div>
          </div>
          <div className="text-center">
            ...
            <div className="text-center">
              {currentResults.noPass?.achievable ? (
                <>
                  ...
                  <div className="text-secondary-text mt-1 text-xs">Results may vary (±1 day)</div>
                </>
              ) : (
                <>
                  <div className="flex items-center justify-center gap-2">
                    <span className="text-lg">{getStatusIcon(false)}</span>
                    <div className="text-lg font-bold text-red-400">Not Achievable</div>
                  </div>
                  <div className="text-secondary-text text-sm">
                    Would complete: {currentResults.noPass?.completionDate}
                  </div>
                  <div className="mt-1 text-xs text-red-400">Past season end date</div>
                </>
              )}
            </div>
          </div>
