...
export default async function ConnectedBotsSection() {
  const [botsData, queueInfo] = await Promise.all([fetchConnectedBots(), fetchQueueInfo()]);
      ...
      <div className="mt-6">
        ...
        <div className="rounded-lg border border-[#2E3944] bg-[#212A31] p-4 shadow-sm">
          <div className="py-8 text-center">
            <div className="mb-2 text-lg font-medium text-gray-400">No bots active</div>
  ...
  if (userIdsToFetch.length > 0) {
    ...
    usersData = fetchedUsersData && 'data' in fetchedUsersData ? null : fetchedUsersData;
