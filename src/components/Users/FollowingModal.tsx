...
const FollowingModal: React.FC<FollowingModalProps> = ({
  ...
  const handleFollowToggle = async (followingId: string) => {
    ...
    try {
      ...
      toast.success(
        isCurrentlyFollowing ? 'Successfully unfollowed user' : 'Successfully followed user',
      );
          ...
          <div className="modal-content max-h-[400px] overflow-y-auto p-6">
            {loading ? (
              ...
              <>
                ...
                {error ? (
                  <div className="text-status-error py-4 text-center text-sm sm:py-8">{error}</div>
