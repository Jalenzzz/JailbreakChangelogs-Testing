...
export default function UserSearchVirtual({ initialUsers }: UserSearchVirtualProps) {
  ...
  return (
    <div className="mb-8 flex flex-col gap-4">
      ...
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <UserTypeTabs userType={userType} onUserTypeChange={(newType) => setUserType(newType)} />
        <div className="text-secondary-text flex items-center gap-2 text-sm">
          <span>
            {(() => {
              const MAX_QUERY_DISPLAY = 32;
              const displayQuery =
                debouncedSearchQuery && debouncedSearchQuery.length > MAX_QUERY_DISPLAY
