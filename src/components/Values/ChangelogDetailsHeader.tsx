...
const ChangelogDetailsHeader: React.FC<ChangelogDetailsHeaderProps> = ({ changelog, userData }) => {
  ...
  return (
    <div className="border-border-primary bg-secondary-bg rounded-lg border p-6">
      <h1 className="text-primary-text mb-2 text-3xl font-bold">Changelog #{changelog.id}</h1>
      <p className="text-secondary-text mb-4">
        {changelog.change_count} change{changelog.change_count !== 1 ? 's' : ''} • Posted on{' '}
              ...
              <div className="relative h-6 w-6 flex-shrink-0 overflow-hidden rounded-full">
                ...
                {userData[discordId]?.avatar && userData[discordId]?.avatar !== 'None' && (
                  <Image
                    ...
                    onError={(e) => {
                      (e as unknown as { currentTarget: HTMLElement }).currentTarget.style.display =
                        'none';
                    }}
                  />
                )}
