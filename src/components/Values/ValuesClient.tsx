...
export default function ValuesClient({ itemsPromise, lastUpdatedPromise }: ValuesClientProps) {
  ...
  useEffect(() => {
    ...
    if (urlFilterSort || urlValueSort) {
      ...
      const savedFilterSort = safeLocalStorage.getItem('valuesFilterSort') as FilterSort;
      const savedValueSort = safeLocalStorage.getItem('valuesValueSort') as ValueSort;
    ...
    if (user && user.id) {
      try {
        ...
        if (favoritesData !== null && Array.isArray(favoritesData)) {
          const favoriteIds = favoritesData.map((fav: FavoriteItem) => parseInt(fav.item_id, 10));
    ...
    <ValuesErrorBoundary>
      ...
      <div className="bg-secondary-bg border-border-primary mb-8 rounded-lg border p-6">
        ...
        <p className="text-secondary-text mb-4">
          Welcome to our Roblox Jailbreak trading values database. We&apos;ve partnered with{' '}
          <a
          ...
          to bring you the most accurate and up-to-date values for all tradeable items in Roblox
          Jailbreak, from limited vehicles to rare cosmetics.
        </p>
      ...
      <HyperchromeCalculatorModal open={showHcModal} onClose={handleCloseHcModal} />
