...
export const TradeAdForm: React.FC<TradeAdFormProps> = ({
  ...
  useEffect(() => {
    if (user) {
      setUserData(user);
      const tier = PREMIUM_TIERS.find((t) => t.tier === user.premiumtype) || PREMIUM_TIERS[0];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const endpoint = editMode ? `/api/trades/update?id=${tradeAd?.id}` : `/api/trades/add`;
      const method = 'POST';
  ...
  if (!isAuthenticated) {
    return (
      <div className="mb-8 rounded-lg border p-6 text-center">
        <h3 className="text-tertiary-text mb-4 text-lg font-medium">Create Trade Ads</h3>
        <p className="text-tertiary-text/70 mb-8">Please log in to create your own trade ads.</p>
      </div>
    );
  }
      ...
      <div className="space-y-6">
        ...
        {!editMode && (
          <div className="border-border-primary bg-button-info/10 mb-2 flex items-start gap-4 rounded-lg border p-4 shadow-sm">
            <div className="relative z-10">
              <span className="text-primary-text text-base font-bold">Trade Ad Expiration</span>
              <div className="text-secondary-text mt-1">
                How long should your trade ad be visible? Supporters can choose longer durations!
                <br />
                <Link
                  href="/supporting"
        ...
        {editMode && tradeAd && (
          <div className="border-border-primary bg-secondary-bg mt-4 rounded-lg border p-4">
            <h3 className="text-tertiary-text mb-4 font-medium">Trade Status</h3>
            {selectLoaded ? (
              <Select
                value={{
          ...
          <div className="border-status-success bg-secondary-bg flex-1 rounded-lg border p-4">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-secondary-text/70 text-sm">({offeringItems.length}/8)</span>
              </div>
              <Tooltip
          ...
          <div className="border-status-error bg-secondary-bg flex-1 rounded-lg border p-4">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-secondary-text/70 text-sm">({requestingItems.length}/8)</span>
              </div>
              <Tooltip
        ...
        <div className="flex flex-col justify-end gap-3 sm:flex-row">
          <button
            ...
            className={`flex items-center gap-2 rounded-md border px-4 py-2 text-sm font-medium transition-colors ${
              !editMode && offeringItems.length === 0 && requestingItems.length === 0
