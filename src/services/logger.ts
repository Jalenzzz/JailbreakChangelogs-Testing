...
class Logger {
   ...
   */
  public apiError(endpoint: string, status: number, message: string, context?: LogContext): void {
   ...
   */
  public dataFetch(operation: string, count: number, context?: LogContext): void {
...
export const logDataFetch = (operation: string, count: number, context?: LogContext) =>
  logger.dataFetch(operation, count, context);

export const logWebSocket = (event: string, status: string) => logger.websocket(event, status);
