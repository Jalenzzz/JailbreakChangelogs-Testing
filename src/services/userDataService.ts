...
export class UserDataService {
  ...
  static async fetchUserData(
    ...
    const [allUserData, allAvatarData, userConnectionData, dupeData] = results.map(
      (result, index) => {
        if (result.status === 'fulfilled') {
          return result.value;
        } else {
          console.warn(`[SERVER] UserDataService: Promise ${index} failed:`, result.reason);
   ...
   */
  private static processUserData(allUserData: unknown): Record<string, RobloxUser> {
    // Process the API response to match our RobloxUser interface
    const robloxUsers: Record<string, RobloxUser> = {};

    if (allUserData && typeof allUserData === 'object' && !Array.isArray(allUserData)) {
   ...
   */
  private static processAvatarData(allAvatarData: unknown): Record<string, string> {
    const robloxAvatars: Record<string, string> = {};

    if (allAvatarData && typeof allAvatarData === 'object' && !Array.isArray(allAvatarData)) {
   ...
   */
  private static processUserConnectionData(userConnectionData: unknown): UserConnectionData | null {
   ...
   */
  private static processDupeData(dupeData: unknown): DupeFinderResponse | undefined {
   ...
   */
  static extractUserIdsFromInventory(inventoryData: InventoryData, robloxId: string): string[] {
   ...
   */
  static extractUserIdsFromDupeData(dupeData: DupeFinderResponse, robloxId: string): string[] {
    ...
    if (dupeData && Array.isArray(dupeData)) {
      ...
      dupeData.forEach((item) => {
        if (item.history) {
          try {
            const historyData =
              typeof item.history === 'string' ? JSON.parse(item.history) : item.history;
   ...
   */
  static extractUserIdsFromOGData(ogData: OGSearchData, robloxId: string): string[] {
    const userIdsToFetch = new Set<string>();
  ...
  static prioritizeUsersByFrequency(
    ...
    logInfo(`Reduced from ${userIds.length} to ${finalUserIds.length} users for performance.`, {
      component: context,
