...
export const getEffectiveCashValue = (item: Item): string => {
  ...
  if (item.children && item.children.length > 0) {
    ...
    const defaultVariant =
      sortedChildren.find((child) => child.sub_name === '2023') || sortedChildren[0];
  ...
  switch (valueSort) {
    ...
    case 'cash-desc':
      result = result.sort((a, b) =>
        sortByCashValue(getEffectiveCashValue(a), getEffectiveCashValue(b), 'desc'),
      );
      break;
    case 'cash-asc':
      result = result.sort((a, b) =>
        sortByCashValue(getEffectiveCashValue(a), getEffectiveCashValue(b), 'asc'),
      );
      break;
    case 'duped-desc':
      result = result.sort((a, b) =>
        sortByCashValue(getEffectiveDupedValue(a), getEffectiveDupedValue(b), 'desc'),
      );
      break;
    case 'duped-asc':
      result = result.sort((a, b) =>
        sortByCashValue(getEffectiveDupedValue(a), getEffectiveDupedValue(b), 'asc'),
      );
      break;
    case 'demand-desc':
      result = result.sort((a, b) =>
